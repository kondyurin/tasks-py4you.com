# Написать скрипт проверки позиций сайта.
# Скрипт считывает запросы из файла. Домен задаётся в консоли.
...
def get_pos_num(keyword, domain):
    """
    Функция получает на вход ключевое слово, домен,
    возвращает позицию, ключ, url
